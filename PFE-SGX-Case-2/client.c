...
int main(int argc , char *argv[])
{
  ...
  com_key=10000+atoi(argv[3]);  // initialize key 
  ...
  for (int i=0;i<iterations;i++){
    ...
    Encrypt_Com(message, 1 , inputsize, 1, com_key);
    //printf("Sent Message:%s\n",message);
    //Encrypt_Com(message, 1 , 16 , 2, 10000);
    //printf("Sent Message:%s\n",message);
    //Encrypt(message,1,5,1);   // encrypt before sending
    //Encrypt(message,1,inputsize,2); 
    com_key++;
